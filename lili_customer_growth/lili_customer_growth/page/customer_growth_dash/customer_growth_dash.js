frappe.pages["customer_growth_dash"].on_page_load = function (wrapper) {
    ...
    $(page.body).html(`
        ...
        <div class="row">
            <div class="col-md-3">
                <div class="tech-card text-center">
                    <div class="kpi-title">累计客户</div>
                    <div class="kpi-number" id="total_customer">0</div>
                    <div class="kpi-rate">客户资产规模</div>
                </div>
            </div>

            <div class="col-md-3">
                <div class="tech-card text-center">
                    <div class="kpi-title">活跃客户</div>
                    <div class="kpi-number" id="active_customer">0</div>
                    <div class="kpi-rate">实时运营</div>
                </div>
            </div>
            ...
            <div class="col-md-3">
                <div class="tech-card text-center">
                    ...
                    <div class="kpi-number" id="monthly_new">0</div>
                    <div class="kpi-rate">增长动力</div>
                </div>
            </div>

            <div class="col-md-3">
                <div class="tech-card text-center">
                    <div class="kpi-title">流失客户</div>
                    <div class="kpi-number" id="lost_customer">0</div>
                    <div class="kpi-rate">风险监控</div>
                </div>
            </div>
        </div>

        <div class="tech-card">
            <div class="chart-title">📈 客户生命周期增长趋势</div>
        ...
        <div class="row">
            ...
            <div class="col-md-6">
                <div class="tech-card">
                    <div class="chart-title">🌏 全国客户分布</div>
        ...
        <div class="tech-card">
            <div class="chart-title">🔋 电池型号使用情况</div>
            <div id="battery_chart"></div>
        </div>

        <div class="tech-card">
            <div class="chart-title">🤖 智能运营洞察</div>
            <div class="insight" id="ai_insight">正在分析数据...</div>
        </div>
    ...
    // 显示当前页面加载时间
    $("#update_time").text(new Date().toLocaleString());
    ...
    frappe.call({
        ...
        callback: function (r) {
            ...
            if (data) {
                animateNumber("#total_customer", data.total_customer);
                animateNumber("#active_customer", data.active_customer);
                animateNumber("#monthly_new", data.monthly_new);
                animateNumber("#lost_customer", data.lost_customer);
            }
        },
    });
    ...
    frappe.call({
        ...
        callback: function (r) {
            ...
            new frappe.Chart("#battery_chart", {
                title: "电池型号使用情况",
                data: {
                    labels: data.map((d) => d.battery_model),
                    datasets: [
                        {
                            name: "使用数量",
                            values: data.map((d) => d.total),
